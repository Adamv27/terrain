...
const getFillColor = (altitude, rainfall) => {
  ...
  if (altitude > terrain.mountainLow) {
    if (altitude > terrain.snow) return colors.mountain.snow;
    if (altitude > terrain.mountainHigh) return colors.mountain.high;
    return colors.mountain.low;
  } 

  if (altitude > terrain.forest) return colors.forest;
